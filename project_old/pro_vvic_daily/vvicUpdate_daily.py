...
T = 3600
while True:
    #更新vvic_daily_test的hashcode
    db = pymysql.connect(host='localhost', port=3306, user='wwj', passwd='Shalou-2018', db='vvic', charset='utf8')
    cursor = db.cursor()
    sql = "SELECT id,images FROM vvic_daily_test where hashcode = '[]' or hashcode is null"#is null
    cursor.execute(sql)
    ...
    for item in results:
        ...
        sql = "update vvic_daily_test set vvic_daily_test.hashcode = '%s'where id= %d"%(hashcode,item[0])
        cursor.execute(sql)
        db.commit()
    cursor.close()
    db.close()
