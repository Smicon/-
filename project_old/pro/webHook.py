...
def mysql_gethash():
    ...
    cursor = db.cursor()
    sql = "SELECT id,hashcode,images FROM vvic_daily where hashcode != '[]'  or hashcode is not null"  # is null
    cursor.execute(sql)
...
class WebHookHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if(generateMD5(url + USER_SECRET + timestamp) == sign2):
            ...
            if data_key.split(':')[0] in TBSPIDER:#来自淘宝店铺
